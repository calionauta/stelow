...
| **Git-primitive coordination at scale** | [Building a C compiler with a team of parallel Claudes](https://www.anthropic.com/engineering/building-c-compiler) (Anthropic, Feb 2026) | Demonstration, not controlled evidence (no single-agent baseline): ~16 agents, ~2,000 sessions, ~$20k coordinated through git lock-files with test validation | `scripts/stelow lock` borrows the lock-file coordination idea; worktree isolation deliberately not adopted — merge burden judged disproportionate to 2–3-scope risk (see [scope-execution-strategy.md](docs/scope-execution-strategy.md)) |
