...
The orchestrator (`stelow-adapter-cli`) is the only `product` skill that is not a sub-skill — it composes the others. The remaining 7 `product` skills plus the 12 `research`, 4 `code`, and 1 `meta` skill are sub-skills (24 total).
...
| **`/sw-*` slash commands (19 Pi / 16 Fusion)** | ✅ All 19 native | ⚠️ Skill delegation via `~/.agents/skills/stelow-adapter-cli` (no native command registration) |
