 ...
 * `skills/stelow-adapter-cli/references/cli-tools/` to each
...
const SOURCE_DIR = join(PROJECT_ROOT, 'skills/stelow-adapter-cli/references/cli-tools');
...
function listSkills(): string[] {
  return readdirSync(SKILLS_DIR).filter((d) => {
    ...
    return statSync(p).isDirectory() && d !== 'stelow-adapter-cli';
  });
}
...
describe('SW-005 isolated temp-fixture regressions', () => {
  ...
  beforeAll(() => {
    ...
    //     skills/
    //       stelow-adapter-cli/references/cli-tools/
    //         subagents.md
    //         goals.md
    ...
    fixtureSource = join(
      fixtureRoot,
      'skills/stelow-adapter-cli/references/cli-tools',
    );
    fixtureTarget = join(
      fixtureRoot,
