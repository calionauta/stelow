...
- `manifest.json` — `id: "fusion-plugin-stelow"`, `name`, `version`,
  ...
  canonical schema.
- `src/agent-installation.ts` — copy vendored `stelow-adapter-cli`
...
1. Copy `skills/stelow-adapter-cli/` + 24 partner skills from
...
1. **Skill trees** — `stelow-adapter-cli` + 24 partner skills,
