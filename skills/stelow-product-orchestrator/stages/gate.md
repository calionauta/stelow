...
| Review Mode | visual review Gate | Interface Gate (int-gate) | Tech Plan Gate (plan-gate) | Code Diff Gate (diff-gate) |
|-------------|:---:|:---:|:---:|:---:|
...
write .stelow/approvals/{_dir}/spec-product.approved.md
approved: true
approved_via: auto (Review Mode=Auto)
```
Then proceed past the gate — no visual review review needed.

**If Review Mode > Auto:** Proceed to gate:5 Claim Verification below.

### gate:5 — Claim Verification (before the Gate)

**BEFORE submitting to visual review**, run claim verification:
...
```
visual_review filePath=.stelow/{YYYY-MM-DD}/{_dir}/plans/spec-product_v{N}.md
```
...
```bash
visual_review annotate .stelow/{YYYY-MM-DD}/{_dir}/plans/spec-product_v{N}.md --gate --json
```
...
approved_via: visual_review --gate
---
```

2. **Receipt auto-created** by the `visual_review` tool at `.stelow/approvals/{_dir}/gate-approved.md` — this triggers stelow auto-advance.
...
3. Re-submit using the `visual_review` tool with the same filePath
4. Repeat until approved

### Model Provenance Check

Before submitting to visual review, determine which model generated each artifact:
...
visual review's visual review catches flow and affordance issues, but
...
2. **visual review gate is MANDATORY** when Review Mode > Auto. Use the `visual_review` tool. Only proceed after `decision: "approved"`.
3. If the reviewer requests changes, adjust and re-submit.
4. After approval, spec-product.md is frozen.
5. **Review Mode = Auto** skips visual review entirely — claim verification still runs if code references exist.

> 💡 **visual review gate is interactive:** you annotate specific lines/paragraphs,
