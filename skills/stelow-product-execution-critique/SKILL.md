...
**Standalone awareness:** when inside stelow, reads scope + mode from `.stelow/*/plans/spec-tech*.md` and `stelow.json` (with fallback to `.stelow/*/index.json` for legacy workflows pre-v0.50.0). When standalone, auto-detects input type (plan path, directory, URL, or nothing). The `HAS_WORKFLOW_DIR` flag gates stelow-specific features; all audit criteria work in both modes.
