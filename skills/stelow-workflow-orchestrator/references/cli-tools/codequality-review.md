...
Save the report to the stelow verification path above so `stelow-workflow-execution-critique` can consume it.

## Fallback (not installed)

If `thermo-nuclear-code-quality-review` is not installed:

- run the normal `code-quality-gate` checks from `skills/stelow-workflow-orchestrator/stages/verification.md`
