...
**Related skills:** `stelow-workflow-interface-alternatives` generates 5 proposal alternatives (Work Pattern Declaration, Design Smell Audit, State Coverage Table). This audit dimension complements that generation workflow — use after proposals are created, or use standalone against existing implementations.
