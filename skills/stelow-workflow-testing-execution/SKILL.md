---
name: stelow-workflow-testing-execution
...
Use `stelow-workflow-ux-critique` for a focused UX/UI quality audit.

**Input routing:**
- **Live URL** → `stelow-workflow-ux-critique` in Live Site mode (full browser audit)
- **Source code** → `stelow-workflow-ux-critique` in Codebase mode (static analysis, ~80% coverage)
- **Screenshot** → `stelow-workflow-ux-critique` in Screenshot mode (quick visual check)
...
2. Run: `stelow-workflow-ux-critique` (URL mode) → found contrast issue on chart labels, high cognitive load on filters
3. Run: `npm test`
