---
name: stelow-adapter-cli
description: "[stelow] Complete product planning workflow CLI adapter: Shape Up -> Interface -> Tech Planning -> Product Critique -> visual review Gate. Includes domain playbooks (Pricing, Trust, Ads, Health, Promotions, Testimonials, Experimentation, Launch Validation, etc)"
metadata:
  frequency: daily
  category: product
  context-cost: medium
...
|---|---|---|
| **Ads** | `stelow-product-ads` | Awareness stages, ad categories, channels & methods |
| **Audience & Market** | `stelow-product-audience-market` | SEDIF model, interviews, JTBD market definition, GPN story |
| **Business Models** | `stelow-product-business-models` | Cost reduction, revenue generation |
| **Experimentation** | `stelow-product-experimentation` | 4 criteria + 6 principles for reliable experimentation |
| **Health** | `stelow-product-health` | Signals in tension, success vs counterbalance |
| **Innovation Strategies** | `stelow-product-innovation-strategies` | Differentiated, Dominant, Sustaining, Discreet, Disruptive |
| **Launch Validation** | `stelow-product-launch-validation` | Pioneer program, pre-sale, waitlist, fake door |
| **Manual Process** | `stelow-product-manual-process` | Concierge, Wizard of Oz, high-risk systems |
...
| **Promotions** | `stelow-product-promotions` | MAGIC framework, 4 launch strategies, bonuses |
| **Solutions Prioritization** | `stelow-product-solutions-prioritization` | 9 components for desirable solutions + impact/risk matrix |
| **Testimonials** | `stelow-product-testimonials` | First 5 / Work for Free, BAB storytelling, collection system |
| **Trust Building** | `stelow-product-trust-building` | 10 pillars, guarantees, trust pact |
...
- Use `references/cli-tools/visual_review.md` for visual review gate rules.

### Scope Adjustment (Scope stage)
- Use **Pattern 3** from `stages/ask-patterns.md`
- No visual review re-run after scope changes — ask tool confirms selections
...
- Use `references/cli-tools/visual_review.md` for visual review command
...
| `context` | **Strategic Context** (optional) | Strategic exploration + domain detection. See `context:5` (appetite/review mode gate), `context:10` (Strategic Approaches — 5 options), `context:20` (Domain Libraries) | — |
...
| `diff-gate` | **Code Diff Review (visual review)** | Visual diff review of working tree. Only in Code Diff mode | After verification |
...
critique — Product Critique (pre-flight)
  ↓
gate — visual review Gate ← visual pause
  ↓
...
planning — Tech Planning*
  ...
  ↓
plan-gate — Tech Plan Gate (visual review) ← conditional: only Tech Review / Code Diff modes
  ↓
execution — Execution
  ↓
verification — Verification (test suite, review, UI audit)
  ↓
diff-gate — Code Diff Review (visual review) ← conditional: only Code Diff mode
  ↓
