...
# Practical Applications: Metrics and First Stepping Stones

When transforming a validated manual process into the first version of a product, the path is rarely a straight line. It looks more like **crossing a river by stepping on stones** — each step reveals the best position for the next one.

## Reflection Questions for the First Version

- **Flexibility and exaptation**: does this first version allow uses we didn't predict? Can it be adapted to solve other problems in the future?
- **Connection potential**: does this solution serve as a foundation and facilitate the development of other features?
- **Ecosystem (gene flow)**: how easy would it be to connect to other tools the audience already uses?
- **Competitive advantage (local competition)**: does this approach position us in a specific niche that is difficult to imitate?

## Approaches for the First Stepping Stone

**Systematize the Core Value ("Minimum Version")**
Automate only the most critical part of the process — the one that generates the greatest perceived value. Release a system with a single essential feature to a small group.

**Build the Acquisition Engine**
Instead of the product itself, build a feature whose main purpose is to attract new customers: a referral system, shareable content, or a free tool that serves as a lure.

**Create an Internal Support Tool**
Automate a painful part of your own manual process first. Increases efficiency and margin, and can eventually become a product for customers.

**Develop in Partnership ("Pilot Solution")**
Find a strategic client and co-develop the first version focused on their needs. Validates viability in a real scenario with guaranteed initial revenue.

## Metrics to Observe

When the system is in operation, monitor:

- **Proxy metric**: the main action that indicates users are extracting value (e.g., % of new customers who create their first project in the first week).
- **Retention rate**: how many customers continue using the solution over time.
- **CLTV/CAC ratio**: is the value generated by a customer greater than the cost to acquire them?
- **Viral coefficient**: are current customers bringing new customers?

These are not goals — they are lenses to understand what's happening and guide the next stepping stone.
...
- `stelow-product-manual-process`
  for discovering the manual process before building

- `stelow-product-launch-validation`
  for pre-sale and commitment testing
