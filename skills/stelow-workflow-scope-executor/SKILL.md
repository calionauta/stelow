...
- **Dependent scopes must wait.** If SCOPE-2 depends on SCOPE-1, do not start SCOPE-2 until SCOPE-1 is complete and reviewed. (`sync-scopes` refuses `blockedBy` cycles at ingest — a cycle fails loud instead of stalling.)
- **Cohesion check before dispatch (cymbal when available).** `target_files` intersection is a coarse floor, not a partition: two scopes can share no declared files yet collide transitively (helpers, fixtures, config). When cymbal is installed, verify transitive disjointness (`cymbal refs` + `cymbal impact` over the union of `target_files`) before parallel dispatch; any shared transitive symbol sequentializes that pair. When cymbal is absent, `target_files` intersection + the Layer-3 post-hoc audit is the fallback — never an excuse to skip the check where the tool exists. (Co-Coder: gains come from cohesion-aware partitioning; naive file-parallel is worse than sequential.)
- **Parent-owned, test-gated merge.** Parallel scopes sharing one checkout never negotiate merges between themselves. The parent merges each scope's output and runs that scope's contract verify commands on the merged tree. Merge conflict or verify failure → human decision (merge, sequential re-run, or rework), same three exits as the Step 8 report. Scopes in host-provisioned isolated worktrees skip mutual exclusion (isolation replaces locking) but keep the parent merge + test gate — the merged tree is what `audit-trail` attests.
- **Regression baseline.** At scope start (Step 3c), run the scope's verify commands once against the unmodified tree and record exit codes in the Record `### Baseline` section (best-effort; commands that cannot run pre-change go in Limitations). Step 8 compares baseline vs close: any newly failing check is a gap blocking advance, same as class (a)/(b). Detail: `references/records-and-tasks.md`, `references/close-out-report.md`.
- **Measure every parallel batch (timing + cost).** The Record mirror carries `started_at`, `finished_at`, `duration_s` (computed, never hand-written) and `cost` (host-reported tokens only — omit the field when unknown, never estimate). No measurement, no claim about parallel payoff.
